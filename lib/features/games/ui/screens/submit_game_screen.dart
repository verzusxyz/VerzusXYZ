...
class _SubmitGameScreenState extends ConsumerState<SubmitGameScreen> {
  ...
  Future<void> _launchSelected(DetectedAppInfo app) async {
    ...
    final game = GameModel(
      gameId: 'temp', title: app.name, platform: platform, packageId: app.packageId,
      bundleId: app.bundleId, webUrl: null, iconUrl: null, defaultCropData: null,
      autoGenEnabled: true, popularityScore: 0, supportsRoomUrl: false,
      supportsRoomCode: false, supportsBoardState: false, roomIdPatterns: const [],
      createdAt: DateTime.now(), approvedBy: null,
    );
    ...
    try {
      if (_selectedDetected.isNotEmpty) {
        ...
        for (final app in _selectedDetected) {
          final platform = (app.packageId?.isNotEmpty ?? false) ? 'android' : (app.bundleId?.isNotEmpty ?? false) ? 'ios' : (defaultTargetPlatform == TargetPlatform.iOS) ? 'ios' : 'android';
          final game = GameModel(gameId: 'temp', title: app.name, platform: platform,
            packageId: app.packageId, bundleId: app.bundleId, webUrl: null, iconUrl: null,
            defaultCropData: null, autoGenEnabled: true, popularityScore: 0,
            supportsRoomUrl: false, supportsRoomCode: false, supportsBoardState: false,
            roomIdPatterns: const [], createdAt: now, approvedBy: null,
          );
        ...
        if (_manualPlatform == 'web') {
          ...
          if (webUrl.isEmpty) { _showError('Enter game link for web'); return; }
        } else if (_manualPlatform == 'android') {
          packageId = _packageIdCtrl.text.trim();
          if (packageId.isEmpty) { _showError('Enter package ID for Android'); return; }
        } else if (_manualPlatform == 'ios') {
          bundleId = _bundleIdCtrl.text.trim();
          if (bundleId.isEmpty) { _showError('Enter bundle ID for iOS'); return; }
        }
        final game = GameModel( gameId: 'temp', title: name, platform: _manualPlatform, packageId: packageId,
          bundleId: bundleId, webUrl: webUrl, iconUrl: null, defaultCropData: null, autoGenEnabled: true,
          popularityScore: 0, supportsRoomUrl: _manualPlatform == 'web', supportsRoomCode: false,
          supportsBoardState: false, roomIdPatterns: const [], createdAt: DateTime.now(), approvedBy: null,
        );
        await service.upsertGameByCanonicalKey(game);
      }
      if (!mounted) return;
      context.pop();
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Game(s) saved')));
    } catch (e) {
          ...
          child: Center(
            child: ConstrainedBox(
              ...
              child: Column(
                ...
                children: [
                  Text('Add Games', style: theme.textTheme.titleLarge?.copyWith(
                    fontWeight: FontWeight.w800,
                    fontSize: responsive.diagonalPercent(0.025)
                  )),
                  SizedBox(height: responsive.heightPercent(0.01)),
                  Text(
                    _isMobile ? 'Select installed games (Android/iOS), or add manually.' : 'Add a game manually (web/desktop).',
                    style: theme.textTheme.bodySmall?.copyWith(fontSize: responsive.diagonalPercent(0.016)),
                  ),
  ...
  Widget _buildSectionHeader(BuildContext context, Responsive responsive, String title) {
  ...
  Widget _buildDivider(BuildContext context, Responsive responsive) {
    return Row(
      children: [
        ...
        Padding(
          padding: EdgeInsets.symmetric(horizontal: responsive.widthPercent(0.02)),
  ...
  Widget _buildDetectedAppsList(BuildContext context, Responsive responsive) {
    ...
    if (_detectedApps.isEmpty) {
      return Container(
        ...
        child: const Text('No supported games detected. Use manual entry below.'),
      );
    }
    return Column(
      children: [
        ..._detectedApps.map((app) => _buildAppListItem(context, responsive, app)),
        if (_selectedDetected.isNotEmpty) ...[
          SizedBox(height: responsive.heightPercent(0.01)),
  ...
  Widget _buildAppListItem(BuildContext context, Responsive responsive, DetectedAppInfo app) {
    final theme = Theme.of(context);
    return Container(
      ...
      child: ListTile(
        leading: app.icon != null
            ...
            : CircleAvatar(
                ...
                child: Icon(Icons.videogame_asset, color: theme.colorScheme.primary),
              ),
        title: Text(app.name, style: TextStyle(fontSize: responsive.diagonalPercent(0.018))),
        subtitle: Text(app.packageId ?? app.bundleId ?? '', style: TextStyle(fontSize: responsive.diagonalPercent(0.014))),
        trailing: Checkbox(
    ...
    return Column(
      children: [
        DropdownButtonFormField<String>(
          ...
          items: ['web', 'android', 'ios'].map((p) => DropdownMenuItem(value: p, child: Text(p.toUpperCase()))).toList(),
          onChanged: (value) {
        ...
        SizedBox(height: responsive.heightPercent(0.015)),
        if (_manualPlatform == 'web')
          VerzusTextField(controller: _webUrlCtrl, label: 'Game Link (e.g., https://chess.com/...)'),
        if (_manualPlatform == 'android')
          VerzusTextField(controller: _packageIdCtrl, label: 'Android Package ID (e.g., com.example.game)'),
        if (_manualPlatform == 'ios')
          VerzusTextField(controller: _bundleIdCtrl, label: 'iOS Bundle ID (e.g., com.example.game)'),
      ],
    );
  }
}
