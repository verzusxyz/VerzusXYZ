...
class MatchModel {
  ...
  final MatchType
      matchType; // Legacy high-level type (quickPlay/skillBased/private/tournament)
  ...
  final WalletKind walletKind; // Indicates if Live or Demo wallet is used
  ...
  factory MatchModel.fromFirestore(DocumentSnapshot doc) {
    ...
    return MatchModel(
      ...
      matchFormat: MatchFormat.values.firstWhere(
        (e) =>
            e.name ==
            (data[MatchDocument.matchFormat] ?? MatchFormat.oneVOne.name),
      ...
      startTime:
          FirestoreHelpers.timestampToDateTime(data[MatchDocument.startTime]),
      endTime:
          FirestoreHelpers.timestampToDateTime(data[MatchDocument.endTime]),
      gameData: data[MatchDocument.gameData],
      participants:
          (data[MatchDocument.participants] as List<dynamic>? ?? const [])
              .map((e) => e.toString())
              .toList(),
      ...
      createdAt:
          FirestoreHelpers.timestampToDateTime(data[MatchDocument.createdAt]) ??
              DateTime.now(),
      updatedAt:
          FirestoreHelpers.timestampToDateTime(data[MatchDocument.updatedAt]) ??
              DateTime.now(),
  ...
  Map<String, dynamic> toFirestore() {
    return {
      ...
      MatchDocument.startTime: startTime != null
          ? FirestoreHelpers.dateTimeToTimestamp(startTime!)
          : null,
      MatchDocument.endTime: endTime != null
          ? FirestoreHelpers.dateTimeToTimestamp(endTime!)
          : null,
...
extension MatchStatusX on MatchStatus {
  ...
  bool get isFinalized =>
      [MatchStatus.completed, MatchStatus.cancelled].contains(this);
