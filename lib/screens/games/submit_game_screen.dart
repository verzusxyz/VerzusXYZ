...
//   DetectedAppInfo? _selectedDetected;
...
//     if (_selectedDetected == null) return;
//     final sel = _selectedDetected!;
//     final platform = (sel.packageId != null && sel.packageId!.isNotEmpty)
//         ? 'android'
//         : (sel.bundleId != null && sel.bundleId!.isNotEmpty)
//             ? 'ios'
...
//     // Build game model either from detected selection or manual inputs
//     GameModel gameToSave;

//     if (_selectedDetected != null) {
//       final sel = _selectedDetected!;
//       final now = DateTime.now();
//       // Infer platform from presence of packageId/bundleId
//       final platform = (sel.packageId != null && sel.packageId!.isNotEmpty)
//           ? 'android'
//           : (sel.bundleId != null && sel.bundleId!.isNotEmpty)
//               ? 'ios'
...
//                   ? 'ios'
//                   : 'android';
//       gameToSave = GameModel(
//         gameId: 'temp', // will be replaced by canonical key
//         title: sel.name,
//         platform: platform,
//         packageId: sel.packageId,
//         bundleId: sel.bundleId,
...
//         ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Enter game name and link')));
//         return;
//       }
...
//     try {
//       await service.upsertGameByCanonicalKey(gameToSave);
//       if (!mounted) return;
...
//         SnackBar(content: Text('Failed to save game: $e'), backgroundColor: VerzusColors.dangerRed),
//       );
//     } finally {
...
//                       Text(
//                         _isMobile
//                             ? 'Pick an installed game (Android/iOS), or add one manually.'
...
//                       if (_isMobile) ...[
//                         Text('Auto-detected', style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.bold)),
...
//                                     ),
//                                     child: const Text('No supported games detected on this device.'),
//                                   )
//                                 : Column(
//                                     children: [
...
//                                             ),
//                                             title: Text(app.name),
//                                             subtitle: Text((app.packageId != null && app.packageId!.isNotEmpty)
//                                                 ? app.packageId!
//                                                 : (app.bundleId ?? '')),
//                                             trailing: Radio<DetectedAppInfo>(
//                                               value: app,
//                                               groupValue: _selectedDetected,
//                                               onChanged: (val) => setState(() => _selectedDetected = val),
//                                             ),
//                                             onTap: () => setState(() => _selectedDetected = app),
//                                           ),
//                                         ),
...
//                       Text('Manual', style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.bold)),
//                       const SizedBox(height: 8),
//                       TextField(
...
//                         decoration: const InputDecoration(
//                           labelText: 'Game Link (e.g., https://chess.com/...)',
//                           border: OutlineInputBorder(),
//                         ),
//                       ),
...
class _SubmitGameScreenState extends ConsumerState<SubmitGameScreen> {
  ...
  bool _detecting = false;
  List<DetectedAppInfo> _detectedApps = [];
  Set<DetectedAppInfo> _selectedDetected = {};
      ...
      if (_selectedDetected.isNotEmpty) {
        ...
        for (final app in _selectedDetected) {
          ...
          final game = GameModel(
            ...
            createdAt: now,
            approvedBy: null,
            resultType: GameResultType.winLoss, // Default value
            ocrEngine: 'mlkit', // Default value
          );
                    ...
                    children: [
                                      ...
                                      for (final app in _detectedApps)
                                        Container(
                                          ...
                                          child: ListTile(
                                            ...
                                            trailing: Checkbox(
                                              value:
                                                  _selectedDetected.contains(app),
                      ...
                      if (_manualPlatform == 'android')
                        TextField(
                          ...
                          decoration: const InputDecoration(
                            labelText: 'Android Package ID (e.g., com.example.game)',
                            border: OutlineInputBorder(),
                          ),
                        ),
