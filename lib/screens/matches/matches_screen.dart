...
class _MatchesScreenState extends ConsumerState<MatchesScreen> with TickerProviderStateMixin {
  ...
  Widget _buildErrorNotice(Object error, {bool compact = false}) {
    final message = error.toString().trim().isEmpty ? 'Unable to load data. Please try again shortly.' : error.toString();
        ...
        children: [
          ...
          matchesAsync.when(
            data: (list) {
              ...
              return Column(
                children: list
                    .map<Widget>((m) => _MatchCard(match: m, onJoin: () => _joinMatch(m.id)))
                    .toList(),
              );
            },
            loading: () => Column(
  ...
  Widget _buildLiveMatches() {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Spectate & Stake', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
          const SizedBox(height: 12),
          activeAsync.when(
            data: (list) {
              if (list.isEmpty) {
              ...
              return Column(
                children: list
                    .map<Widget>((m) => _LiveMatchCard(match: m, onStake: () => _showStakeDialog(m)))
                    .toList(),
              );
            },
            loading: () => Column(children: List.generate(4, (_) => Padding(padding: const EdgeInsets.only(bottom: 12), child: VerzusShimmers.listTile()))),
            error: (e, _) => _buildErrorNotice(e),
          ),
        ],
      ),
      ...
      children: [
        Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Create Match', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
              const SizedBox(height: 12),
              gamesAsync.when(
                data: (games) {
                  return DropdownButtonFormField<String>(
                    ...
                    onChanged: (val) {
                      setState(() {
                        _selectedGameId = val;
                        final g = games.firstWhere((x) => x.gameId == val, orElse: () => games.first);
              ...
              DropdownButtonFormField<MatchFormat>(
                value: _format,
                items: const [
                  DropdownMenuItem(value: MatchFormat.oneVOne, child: Text('1v1')),
                  DropdownMenuItem(value: MatchFormat.freeForAll, child: Text('Free-for-all')),
                  DropdownMenuItem(value: MatchFormat.teamBased, child: Text('Team-based')),
                ],
                onChanged: (v) => setState(() => _format = v ?? MatchFormat.oneVOne),
                decoration: InputDecoration(
                ...
                child: VerzusButton(
                  onPressed: () async {
                    ...
                    if (_selectedGameId == null) {
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Select a game')));
                      return;
                    }
                    ...
                    try {
                      await ref.read(matchServiceProvider).createMatch(
                        ...
                        wagerAmount: wager,
                        walletKind: _mode == 'Live' ? WalletKind.live : WalletKind.demo,
                        matchFormat: _format,
                        gameData: {
                      ...
                      if (mounted) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text('Failed: $e'), backgroundColor: VerzusColors.dangerRed),
                        );
                      }
                    }
                  },
            ...
            SizedBox(
              ...
              child: VerzusButton(
                onPressed: () async {
                  ...
                  if (wager <= 0) {
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Enter a valid wager amount')));
                    return;
                  }
                  // KYC check if profile loaded
                  if (user != null && user.kycStatus != KycStatus.verified) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Please complete KYC verification to create matches')),
                    );
                    return;
                  }
                  try {
  ...
  Future<void> _joinMatch(String matchId) async {
    ...
    if (auth == null) return;
    try {
      await ref.read(matchServiceProvider).joinMatch(matchId: matchId, opponentId: auth.uid);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to join: $e'), backgroundColor: VerzusColors.dangerRed),
        );
      }
    }
  }
        ...
        content: Column(
          ...
          children: [
            ...
            TextField(
              ...
              keyboardType: const TextInputType.numberWithOptions(decimal: true),
              decoration: const InputDecoration(labelText: 'Stake Amount (USD)', prefixText: '\$'),
            ),
          ],
        ),
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              try {
                ...
                await ref.read(walletServiceProvider).lockFunds(auth.uid, amt, kind: isDemo ? WalletKind.demo : WalletKind.live);
                await FirebaseFirestore.instance.collection('match_stakes').add({
                  'match_id': match.id,
                  'user_id': auth.uid,
                  'amount': amt,
                ...
                if (context.mounted) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Stake placed')));
                }
              } catch (e) {
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text('Wager: \$${match.wagerAmount.toStringAsFixed(2)}', style: Theme.of(context).textTheme.bodySmall?.copyWith(color: colorScheme.onSurfaceVariant)), 
              ],
            ),
          ),
          VerzusButton(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                Text('Started', style: Theme.of(context).textTheme.bodySmall?.copyWith(color: colorScheme.onSurfaceVariant)),
              ],
            ),
          ),
          VerzusButton(
