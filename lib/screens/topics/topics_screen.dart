...
// class _TopicsScreenState extends ConsumerState<TopicsScreen> with TickerProviderStateMixin {
...
//   final entryCtrl = TextEditingController(text: '1.00');
...
//                             questionCtrl.clear();
//                             entryCtrl.text = '1.00';
//                             for (final c in optionCtrls) c.clear();
//                           }
//                         } catch (e) {
//                           if (mounted) {
//                             ScaffoldMessenger.of(context).showSnackBar(
...
//             return _buildErrorNotice(context, snapshot.error ?? 'Unknown error');
//           }
...
//           return Column(
//             children: docs
//                 .map((d) => _PollCard(pollId: d.id, data: d.data()))
//                 .toList(),
//           );
//         },
//       ),
...
//                   title: 'No Open Topics',
//                   subtitle: 'Create the first community topic for others to stake on',
//                 );
//               }
//               return Column(
//                 children: list.map((t) => _TopicCard(title: t[SkillTopicDocument.name] ?? 'Topic')).toList(),
//               );
//             },
//             loading: () => Column(
//               children: [
...
//         subtitle: 'Your stakes on topics will appear here',
//       ),
//     );
//   }
...
//                 try {
//                   await ref.read(topicsServiceProvider).createOpenTopic(title: title, description: desc.isEmpty ? null : desc);
//                   if (mounted) {
...
//         crossAxisAlignment: CrossAxisAlignment.start,
//         children: [
//           Text(question, style: Theme.of(context).textTheme.titleSmall?.copyWith(fontWeight: FontWeight.w700)),
//           const SizedBox(height: 8),
//           Wrap(
//             spacing: 8,
//             runSpacing: 8,
...
//           ),
//           const SizedBox(height: 8),
//           Text('Entry: \$${entry.toStringAsFixed(2)}', style: Theme.of(context).textTheme.bodySmall?.copyWith(color: cs.onSurfaceVariant)),
//         ],
//       ),
//     );
//   }

//   Future<void> _vote(BuildContext context, WidgetRef ref, int optionIndex, double entry) async {
...
//     } catch (e) {
//       if (context.mounted) {
//         ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed: $e'), backgroundColor: VerzusColors.dangerRed));
//       }
//     }
//   }
// }
          ...
          builder: (context, snapshot) {
            ...
            if (docs.isEmpty) {
              return _buildEmptyState(
                ...
                subtitle: 'Spark some excitement—create a poll and invite the community to join in!',
              );
            }
            if (isNarrow) {
              return SingleChildScrollView(
                ...
                data: (list) {
                  if (list == null || list.isEmpty) {
                    return _buildEmptyState(
                      ...
                      subtitle: 'Ignite the conversation—create the first community topic and let the stakes begin!',
                    );
                  }
                  if (isNarrow) {
                    return Column(
                      children: list.map((t) {
                        final title = t is Map<dynamic, dynamic> ? t[SkillTopicDocument.name] as String? ?? 'Topic' : 'Topic';
                        return Padding(
                          padding: const EdgeInsets.only(bottom: 12),
