            ...
            return Padding(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  IconButton(
                    icon: const Icon(Icons.add_circle, color: VerzusColors.primaryPurple),
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      DropdownButtonFormField<String>(
                        ...
                        onChanged: (v) =>
                            setStateLocal(() => bracketType = v ?? 'single_elim'),
                        decoration: InputDecoration(
                          labelText: 'Tournament type',
                          border: OutlineInputBorder(
                            borderRadius: BorderRadius.circular(12),
            ...
            builder: (context, snapshot) {
              ...
              return ListView.separated(
                ...
                itemBuilder: (context, index) {
                  ...
                  final entry = (t[TournamentDocument.entryFee] ?? 0.0).toDouble();
                  final walletKind = (t[TournamentDocument.walletKind] ?? 'live') as String;
                  final title = t[TournamentDocument.title] as String? ?? 'Tournament';
                  final type = t[TournamentDocument.tournamentType] as String? ?? 'Unknown';
        ...
        actions: [
          ...
          VerzusButton(
            onPressed: () async {
              ...
              final s = (tier['entry'] as String);
              final entry =
                  double.tryParse(s.startsWith('\$') ? s.substring(1) : s) ?? 0.0;
              final mode = ref.read(walletModeProvider);
              try {
                if (entry > 0) {
                ...
                final refCol =
                    FirebaseFirestore.instance.collection('tournament_waitlist');
                await refCol.add({
                  'user_id': auth.uid,
                  'tier': tier['entry'],
                  'entry_fee': entry,
