...
class AuthService {
  ...
  String _mapFirebaseAuthError(FirebaseAuthException e, {required bool forSignUp}) {
    switch (e.code) {
  ...
  Future<UserModel?> signUpWithEmailAndPassword({
    ...
    try {
      ...
      await _firestore.runTransaction((txn) async {
        final unameRef = _firestore.collection(FirestoreSchema.usernames).doc(normalized);
        final unameSnap = await txn.get(unameRef);
        if (unameSnap.exists) {
          throw Exception('USERNAME_TAKEN');
        }
        final userRef = _firestore.collection(FirestoreSchema.users).doc(user.uid);
      ...
      if (e.toString().contains('USERNAME_TAKEN')) {
        ...
        throw const AuthException('That username is already taken. Please choose another.');
      }
      // Permission / network / unknown errors
      throw const AuthException('We couldn\'t complete sign up. Please try again.');
    }
  }
  ...
  Future<void> updateUserProfile({
    ...
    try {
      ...
      if (username != null) updates[UserDocument.username] = _normalizeUsername(username);
      ...
      if (updates.isNotEmpty) {
        ...
        await _firestore.collection(FirestoreSchema.users).doc(user.uid).update(updates);
      }
    } catch (e) {
  ...
  Future<bool> isUsernameAvailable(String username) async {
    try {
      final normalized = _normalizeUsername(username);
      final doc = await _firestore.collection(FirestoreSchema.usernames).doc(normalized).get();
      return !doc.exists;
    } catch (e) {
  ...
  Future<void> deleteAccount() async {
    ...
    try {
      ...
      await _firestore.collection(FirestoreSchema.wallets).doc(user.uid).delete();
