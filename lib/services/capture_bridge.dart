...
typedef FrameCallback = FutureOr<void> Function(Uint8List pngBytes, int timestamp);
...
class CaptureBridge {
  ...
  Future<void> startCapture({
    ...
    required Map<String, dynamic> cropRects, // {scoreRect: {x,y,w,h}, usernameRect: {...}, roomRect: {...}}
    required int fps,
    ...
    if (kIsWeb) {
      ...
      await webcap.startWebCapture({'fps': fps, 'cropRects': cropRects, 'gameId': gameId});
      return;
    }

    // For Android/iOS platform channels: we register an EventChannel for frames
    final EventChannel eventChannel = const EventChannel('com.verzusxyz.captureFrames');
