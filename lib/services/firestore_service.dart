...
class FirestoreService {
  ...
  Future<void> createMatch(MatchModel match) async {
    try {
      await _firestore.collection(FirestoreSchema.matches).doc(match.id).set(match.toFirestore());
    } catch (e) {
  ...
  Future<MatchModel?> getMatch(String matchId) async {
    try {
      final doc = await _firestore.collection(FirestoreSchema.matches).doc(matchId).get();
  ...
  Stream<List<MatchModel>> getAvailableMatches({
    ...
    try {
      ...
      throw FirestoreException('Failed to get available matches: ${e.toString()}');
    }
  }
  ...
  Future<void> updateMatchStatus(String matchId, String status, {String? winnerId, String? loserId}) async {
    try {
      ...
      await _firestore.collection(FirestoreSchema.matches).doc(matchId).update(updates);
    } catch (e) {
      throw FirestoreException('Failed to update match status: ${e.toString()}');
    }
  }
  ...
  Future<void> createWalletTransaction({
    ...
    try {
      ...
      await _firestore.collection(FirestoreSchema.walletTransactions).doc(transactionId).set({
  ...
  Stream<List<Map<String, dynamic>>> getUserTransactions(String userId, {int limit = 50}) {
    try {
      ...
      throw FirestoreException('Failed to get user transactions: ${e.toString()}');
    }
  }
  ...
  Future<Map<String, dynamic>?> getSystemSetting(String key) async {
    try {
      final doc = await _firestore.collection(FirestoreSchema.systemSettings).doc(key).get();
  ...
  Future<void> updateUserStats(String userId, {
    ...
    try {
      ...
      if (wins != null) updates[UserDocument.totalWins] = FieldValue.increment(wins);
      if (losses != null) updates[UserDocument.totalLosses] = FieldValue.increment(losses);
      if (skillRatings != null) updates[UserDocument.skillRatings] = skillRatings;

      await _firestore.collection(FirestoreSchema.users).doc(userId).update(updates);
    } catch (e) {
  ...
  Future<void> initializeSkillTopics() async {
    try {
      final topics = [
        {
          ...
          SkillTopicDocument.description: 'Addition, subtraction, multiplication, division',
      ...
      for (final topic in topics) {
        final docRef = _firestore.collection(FirestoreSchema.skillTopics).doc(topic[SkillTopicDocument.id] as String);
        batch.set(docRef, topic);
      }
      await batch.commit();
    } catch (e) {
      throw FirestoreException('Failed to initialize skill topics: ${e.toString()}');
    }
  }
  ...
  Future<void> initializeSystemSettings() async {
    try {
      final settings = [
        ...
        {
          SystemSettingsDocument.key: FirestoreConstants.settingMinWithdrawalAmount,
          SystemSettingsDocument.value: '10.00',
          SystemSettingsDocument.description: 'Minimum withdrawal amount in USD',
          SystemSettingsDocument.updatedAt: FieldValue.serverTimestamp(),
        },
        {
      ...
      for (final setting in settings) {
        final docRef = _firestore.collection(FirestoreSchema.systemSettings).doc(setting[SystemSettingsDocument.key] as String);
        batch.set(docRef, setting);
      }
      await batch.commit();
    } catch (e) {
      throw FirestoreException('Failed to initialize system settings: ${e.toString()}');
    }
  }
}
