...
final gameSubmissionsStreamProvider =
    StreamProvider<List<GameSubmissionModel>>((ref) {
...
class GamesService {
  ...
  CollectionReference get _submissionsCol =>
      _firestore.collection('games_user_submissions');
  CollectionReference _usernameMappingsCol(String uid) =>
      _firestore.collection('username_mappings').doc(uid).collection('games');
  ...
  Stream<List<GameSubmissionModel>> streamSubmissions(
    ...
    return query.snapshots().map((qs) =>
        qs.docs.map((d) => GameSubmissionModel.fromFirestore(d)).toList());
  }
  ...
  Future<void> createSubmissionWithId(
      String id, GameSubmissionModel submission) async {
  ...
  Future<void> approveSubmission(String submissionId,
      {required String approvedBy}) async {
    ...
    final game = GameModel(
      ...
      webUrl: sub.webUrl,
      iconUrl:
          sub.sampleImageUrls.isNotEmpty ? sub.sampleImageUrls.first : null,
    ...
    batch.update(subRef, {
      'status': existing.exists
          ? GameSubmissionStatus.merged.name
          : GameSubmissionStatus.approved.name,
  ...
  Future<void> rejectSubmission(String submissionId,
      {required String reviewedBy, String? notes}) async {
