...
class OCRService {
  ...
  Future<List<String>> extractCandidatesFromPng(Uint8List pngBytes, {Map<String, num>? cropRect}) async {
  ...
  Future<Map<String, dynamic>> suggestCropRects(Uint8List pngBytes) async {
    ...
    try {
      ...
      if (decoded != null) {
        ...
        return {
          'scoreRect': {'x': (w * 0.55).toInt(), 'y': (h * 0.05).toInt(), 'w': (w * 0.4).toInt(), 'h': (h * 0.12).toInt()},
          'usernameRect': {'x': (w * 0.05).toInt(), 'y': (h * 0.80).toInt(), 'w': (w * 0.5).toInt(), 'h': (h * 0.12).toInt()},
          'confidence': 0.2,
        };
      }
    } catch (_) {}
