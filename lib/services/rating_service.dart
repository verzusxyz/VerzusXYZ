...
class RatingService {
  ...
  Future<void> updateRatings(
      String gameId, String winnerId, String loserId) async {
    ...
    await _firestore.runTransaction((transaction) async {
      ...
      final winnerRatings =
          Map<String, double>.from(winnerSnapshot.data()?['ratings'] ?? {});
      final loserRatings =
          Map<String, double>.from(loserSnapshot.data()?['ratings'] ?? {});
      ...
      final expectedWinner =
          1.0 / (1.0 + (10.0 * (loserRating - winnerRating) / 400.0));
      final expectedLoser =
          1.0 / (1.0 + (10.0 * (winnerRating - loserRating) / 400.0));
