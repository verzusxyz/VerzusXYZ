...
class ResultTracker {
  ...
  Future<void> _refundLocked(Transaction txn,
      {required MatchModel match}) async {
    final wallets =
        FirebaseFirestore.instance.collection(FirestoreSchema.wallets);
    ...
    final isDemo =
        match.gameMode == 'demo' || (match.gameData?['mode'] == 'demo');
  ...
  Future<void> _awardWinner(
    ...
    final double platformCut = isDemo
        ? 0.0
        : double.parse((totalPot * platformRate).toStringAsFixed(2));
    final double winnerPayout = isDemo
        ? totalPot
        : double.parse((totalPot - platformCut).toStringAsFixed(2));
    ...
    if (winnerSnap.exists) {
      ...
      if (isDemo) {
        ...
        txn.update(winnerRef, {
          ...
          WalletDocument.totalWon:
              FieldValue.increment(winnerPayout - amountPerPlayer),
          WalletDocument.updatedAt: FieldValue.serverTimestamp(),
        });
      }
    }
