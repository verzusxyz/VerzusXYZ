...
class StakingService {
  ...
  Future<void> createStake({
    ...
    await _firestore.collection('stakes').doc(stake.stakeId).set(stake.toMap());
  }

  Future<void> processStakesForMatch(
      String matchId, String winningPlayerId) async {
    ...
    for (final doc in stakesSnapshot.docs) {
      ...
      final double payout =
          (stake.playerStakedOnId == winningPlayerId) ? stake.amount * 2 : 0;
      ...
      await _firestore.runTransaction((transaction) async {
        ...
        final currentAvailableBalance =
            snapshot.data()?['availableBalance'] ?? 0.0;
...
extension WalletServiceLockFundsForEntry on WalletService {
  Future<void> lockFundsForEntry(String userId, double amount) async {
    final firestore = FirebaseFirestore.instance;
    ...
    await firestore.runTransaction((transaction) async {
      final snapshot = await transaction.get(walletRef);
      if (!snapshot.exists) {
        throw Exception('Wallet for user $userId not found.');
      }

      final data = snapshot.data() ?? {};

      final currentAvailableRaw = data['availableBalance'];
      final double currentAvailable = currentAvailableRaw is num
          ? currentAvailableRaw.toDouble()
          : double.tryParse(currentAvailableRaw?.toString() ?? '') ?? 0.0;

      final currentPendingRaw = data['pendingBalance'];
      final double currentPending = currentPendingRaw is num
          ? currentPendingRaw.toDouble()
          : double.tryParse(currentPendingRaw?.toString() ?? '') ?? 0.0;

      if (currentAvailable < amount) {
        throw Exception(
            'Insufficient available balance to lock $amount for user $userId');
      }
