...
final tournamentManagerProvider = Provider<TournamentManager>((ref) => TournamentManager());
...
class TournamentManager {
  ...
  Future<String> createTournament({
    ...
    final inviteCode = visibility == 'private' ? doc.id.substring(0, 6).toUpperCase() : null;
    final ratios = payoutRatios ?? (payoutMode == 'winner_takes_all' ? {'1': 100} : {'1': 50, '2': 30, '3': 20});
    final Timestamp? regDeadline = startDate != null
        ? Timestamp.fromDate(startDate.subtract(Duration(minutes: checkinDeadlineMins)))
        : null;
    await doc.set({
      ...
      TournamentDocument.startDate: startDate != null ? Timestamp.fromDate(startDate) : FieldValue.serverTimestamp(),
  ...
  Future<void> joinTournament({
    ...
    await _fs.runTransaction((txn) async {
      ...
      if ((t[TournamentDocument.status] as String) != FirestoreConstants.tournamentStatusOpen) {
      ...
      if (entry > 0) {
        await WalletService().lockFunds(userId, entry, kind: isDemo ? WalletKind.demo : WalletKind.live);
      }
      // Add participant row
  ...
  Future<void> generateBracketAndMatches(String tournamentId) async {
    ...
    if (!tSnap.exists) throw Exception('Tournament not found');
    final t = tSnap.data() as Map<String, dynamic>;
    final type = (t[TournamentDocument.tournamentType] ?? 'single_elim') as String;
    final isDemo = (t[TournamentDocument.walletKind] ?? 'live') == 'demo';
    final maxP = (t[TournamentDocument.maxParticipants] ?? 0) as int;
    // Load participants
    final qs = await _fs
        ...
        .get();
    final participants = qs.docs.map((d) => d.data()[TournamentParticipantDocument.userId] as String).toList();
    ...
    if (type == 'single_elim') {
      ...
      for (int i = 0; i < seeded.length; i += 2) {
        ...
        final matchData = {
          ...
          MatchDocument.skillTopic: t[TournamentDocument.skillTopic] ?? 'general',
  ...
  Future<void> payoutTournament(String tournamentId, List<String> placements) async {
    ...
    await _fs.runTransaction((txn) async {
      ...
      if (!tSnap.exists) throw Exception('Tournament not found');
      final t = tSnap.data() as Map<String, dynamic>;
      final totalEntries = (t[TournamentDocument.entryFeesTotal] ?? 0.0).toDouble();
      ...
      for (final d in qs.docs) {
        ...
        if (entry > 0) {
          await WalletService().consumeLocked(uid, entry, kind: isDemo ? WalletKind.demo : WalletKind.live);
        }
      }
      ...
      for (int i = 0; i < placements.length; i++) {
        ...
        await WalletService().creditBalance(winnerId, amount, kind: isDemo ? WalletKind.demo : WalletKind.live);
      ...
      final adminRef = _fs.collection(AdminFinancialsSchema.collection).doc(AdminFinancialsSchema.commissions);
      txn.set(adminRef, {
        AdminFinancialsSchema.totalCommission: FieldValue.increment(platformCut),
  ...
  Future<void> markMatchDispute({required String tournamentId, required String matchId}) async {
