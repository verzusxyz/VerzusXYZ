...
class WalletService {
  ...
  Stream<WalletModel?> watchWallet(String userId) {
    return _firestore.collection(FirestoreSchema.wallets).doc(userId).snapshots().map((doc) {
      try {
  ...
  Future<void> lockFunds(String userId, double amount, {WalletKind kind = WalletKind.live}) async {
    try {
      await _firestore.runTransaction((txn) async {
        ...
        if (kind == WalletKind.live) {
          ...
          final currentPending = (data[WalletDocument.pendingBalance] ?? 0.0).toDouble();
          ...
          final currentPending = (data['demo_pending_balance'] ?? 0.0).toDouble();
          if (currentBalance < amount) {
  ...
  Future<void> unlockFunds(String userId, double amount, {WalletKind kind = WalletKind.live}) async {
    try {
      await _firestore.runTransaction((txn) async {
        final ref = _firestore.collection(FirestoreSchema.wallets).doc(userId);
        final snap = await txn.get(ref);
  ...
  Future<void> consumeLocked(String userId, double amount, {WalletKind kind = WalletKind.live}) async {
    try {
      await _firestore.runTransaction((txn) async {
        final ref = _firestore.collection(FirestoreSchema.wallets).doc(userId);
        final snap = await txn.get(ref);
  ...
  Future<void> creditBalance(String userId, double amount, {WalletKind kind = WalletKind.live}) async {
    try {
      await _firestore.collection(FirestoreSchema.wallets).doc(userId).update({
        (kind == WalletKind.live ? WalletDocument.balance : 'demo_balance'): FieldValue.increment(amount),
        WalletDocument.updatedAt: FieldValue.serverTimestamp(),
      });
    } catch (e) {
      // ignore: avoid_print
  ...
  Future<void> addAffiliatePending(String userId, double amount, {WalletKind kind = WalletKind.live}) async {
    try {
      await _firestore.collection(FirestoreSchema.wallets).doc(userId).update({
        kind == WalletKind.live ? WalletDocument.pendingBalance : 'demo_pending_balance': FieldValue.increment(amount),
        WalletDocument.updatedAt: FieldValue.serverTimestamp(),
      });
    } catch (e) {
      // ignore: avoid_print
  ...
  Future<String> addWalletTransaction({
    ...
    try {
      final ref = _firestore.collection(FirestoreSchema.walletTransactions).doc();
      await ref.set({
...
class WalletNotifier extends Notifier<WalletModel?> {
  ...
  Future<void> lockFunds(double amount, {WalletKind kind = WalletKind.live}) async {
    if (state == null) return;
  ...
  Future<void> unlockFunds(double amount, {WalletKind kind = WalletKind.live}) async {
    if (state == null) return;
  ...
  Future<void> addAffiliatePending(double amount, {WalletKind kind = WalletKind.live}) async {
    if (state == null) return;
    
    try {
      await _walletService.addAffiliatePending(state!.userId, amount, kind: kind);
      if (kind == WalletKind.live) {
        state = state!.copyWith(
          pendingBalance: state!.pendingBalance + amount,
          updatedAt: DateTime.now(),
