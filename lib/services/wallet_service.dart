  ...
  Future<void> processPayouts(
    ...
    await _firestore.runTransaction((transaction) async {
      ...
      for (final userId in participantIds) {
        ...
        if (!snap.exists) throw Exception('Wallet for participant $userId not found.');

        final data = snap.data() as Map<String, dynamic>;
        if (kind == WalletKind.live) {
          final currentPending = (data[WalletDocument.pendingBalance] ?? 0.0).toDouble();
          if (currentPending < entryFee) throw Exception('Insufficient locked funds for $userId');
          ...
          if (currentPending < entryFee) throw Exception('Insufficient locked demo funds for $userId');
          transaction.update(ref, {'demo_pending_balance': currentPending - entryFee});
        }
      }
